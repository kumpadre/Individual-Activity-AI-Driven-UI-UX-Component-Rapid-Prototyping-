...
let reports = [
  { id: 1, name: "Monthly user growth — August", date: "2026-09-01", type: "Growth" },
  { id: 2, name: "Engagement summary — Q3", date: "2026-08-15", type: "Engagement" },
  { id: 3, name: "Churn & inactivity report", date: "2026-08-02", type: "Retention" },
];
let nextReportId = 4;
...
const VIEW_META = {
  dashboard: { title: "Dashboard", subtitle: "Overview of your users and platform activity." },
  analytics: { title: "Analytics", subtitle: "Trends in signups, roles, and engagement." },
  users: { title: "Users", subtitle: "Search, filter, and manage every account." },
  reports: { title: "Reports", subtitle: "Generate and review platform reports." },
  settings: { title: "Settings", subtitle: "Manage your account and notification preferences." },
};
...
function renderStats() {
  const html = statCardsHtml(getStatCards());
  const dashboardGrid = document.getElementById("statsGridDashboard");
  const analyticsGrid = document.getElementById("statsGridAnalytics");
  if (dashboardGrid) dashboardGrid.innerHTML = html;
  if (analyticsGrid) analyticsGrid.innerHTML = html;
}
...
function renderRecentActivity() {
  ...
  if (!container) return;
  const recent = [...users].sort((a, b) => a.lastActiveMinutes - b.lastActiveMinutes).slice(0, 6);
  container.innerHTML = recent.map((u) => `
    <div class="recent-item">
      ${avatarHtml(u, 34)}
      <div>
        <p class="recent-item-name">${escapeHtml(u.name)}</p>
        <p class="recent-item-sub">${escapeHtml(u.role)}</p>
      </div>
      <span class="recent-item-time">${formatRelativeTime(u.lastActiveMinutes)}</span>
    </div>
  `).join("");
}
    ...
    return `
      <div class="breakdown-row">
        <div class="breakdown-top">
          <span class="breakdown-label"><span class="status-dot" style="background:${s.dot}"></span>${status}</span>
          <span>${count} · ${pct}%</span>
        </div>
        <div class="breakdown-track"><div class="breakdown-fill" style="width:${pct}%;background:${s.dot}"></div></div>
      </div>
    `;
  }).join("");
}
...
function renderSignupChart() {
  const container = document.getElementById("signupChart");
  if (!container) return;
  const now = new Date();
  const months = [];
  for (let i = 5; i >= 0; i--) {
  ...
  const counts = months.map((m) => {
    return users.filter((u) => {
      ...
      return joined.getFullYear() === m.getFullYear() && joined.getMonth() === m.getMonth();
    }).length;
  });
  const max = Math.max(1, ...counts);
  container.innerHTML = months.map((m, i) => `
    <div class="bar-chart-col">
      <span class="bar-chart-value">${counts[i]}</span>
      <div class="bar-chart-bar" style="height:${Math.max(4, (counts[i] / max) * 130)}px"></div>
      <span class="bar-chart-label">${m.toLocaleDateString("en-US", { month: "short" })}</span>
    </div>
  `).join("");
}
...
function renderRoleChart() {
  ...
  if (!container) return;
  const roleColors = { Administrator: "#4F46E5", Manager: "#0EA5E9", User: "#16A34A", Editor: "#D97706" };
  const max = Math.max(1, ...ROLES.map((r) => users.filter((u) => u.role === r).length));
  container.innerHTML = ROLES.map((role) => {
    const count = users.filter((u) => u.role === role).length;
    const pct = Math.round((count / max) * 100);
    return `
      <div class="role-chart-row">
        <span class="role-chart-dot" style="background:${roleColors[role]}"></span>
        <span class="role-chart-name">${role}</span>
        <div class="role-chart-track"><div class="role-chart-fill" style="width:${pct}%;background:${roleColors[role]}"></div></div>
        <span class="role-chart-count">${count}</span>
      </div>
    `;
  }).join("");
}
...
function renderReports() {
  ...
  if (!container) return;
  if (reports.length === 0) {
    container.innerHTML = `<p class="report-empty">No reports yet. Generate one to get started.</p>`;
    return;
  }
  container.innerHTML = [...reports].reverse().map((r) => `
    <div class="report-row">
      <div class="report-main">
        ...
        <div>
          <p class="report-name">${escapeHtml(r.name)}</p>
          <p class="report-sub">${r.type} · ${formatJoinedDate(r.date)}</p>
        </div>
      </div>
      <button class="btn btn-ghost" data-download="${r.id}">Download</button>
    </div>
  `).join("");
}
...
function initReports() {
  ...
  document.getElementById("generateReportBtn").addEventListener("click", () => {
    const types = ["Growth", "Engagement", "Retention", "Activity"];
...
function initSettings() {
  ...
  form.addEventListener("submit", (e) => {
    ...
    msg.hidden = false;
    clearTimeout(initSettings._t);
    initSettings._t = setTimeout(() => { msg.hidden = true; }, 2000);
  });
}
...
function switchView(view) {
  ...
  // Icons for newly-revealed markup (e.g. report rows rendered while hidden).
  lucide.createIcons();
}
